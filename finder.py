        ...
        if type(ID) == None:
            ...
            while True:
                if first:
                    first = False
                    try:
                        request = self.youtube.commentThreads().list(part="snippet,id", searchTerms = term, maxResults=100, videoId=ID, textFormat="plainText")
                        response = request.execute()
                        print('found first page and got comments')
                    except:
                        return valid_comments
                else:
                    try:
                        request = self.youtube.commentThreads().list(part="snippet,id", maxResults=100, pageToken=response["nextPageToken"], searchTerms = term, videoId=ID, textFormat="plainText")
                        response = request.execute()
                    except:
                        return valid_comments
